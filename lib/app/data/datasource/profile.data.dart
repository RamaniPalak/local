...
abstract class ProfileData {
  ...
  //
  // Future<ResEmpty> updateUserProfile({required ResGetUserDetailData data});
}
...
class ProfileDataImpl implements ProfileData {
  ...
  // @override
  // Future<ResEmpty> updateUserProfile(
  //     {required Meber data}) async {
  //   final user = await UserPrefs.shared.getUser;
  //
  //   data.memberId = user.memberID;
  //
  //   final req = ReqUpdateUser(
  //       memberId: data.memberId, firstName: data.firstName, eMail: data.eMail);
  //
  //   final res = await WebService.shared
  //       .postApiDIO(path: ServerConfigs.updateUserProfile, data: req);
  //
  //   try {
  //     return ResEmpty.fromJson(res!);
  //   } catch (e) {
